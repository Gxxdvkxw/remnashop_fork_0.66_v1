...
class UserService(BaseService):
    ...
    async def create(self, aiogram_user: AiogramUser) -> UserDto:
        ...
        await self.uow.repository.create_instance(db_user)
        return db_user.dto()

    @redis_cache(prefix="get_user", ttl=TIME_1M)
    async def get(self, telegram_id: int) -> Optional[UserDto]:
        user = await self.uow.repository.users.get(telegram_id=telegram_id)
        return user.dto() if user else None

    async def update(self, user: UserDto) -> Optional[UserDto]:
        db_user = await self.uow.repository.users.get(telegram_id=user.telegram_id)

        if not db_user:
            return None

        db_user = await self.uow.repository.users.update(
            user.telegram_id,
            **user.model_state,
        )
    ...
    async def delete(self, user: UserDto) -> bool:
        result = await self.uow.repository.users.delete(telegram_id=user.telegram_id)
    ...
    async def get_by_partial_name(self, query: str) -> list[UserDto]:
        users = await self.uow.repository.users.get_by_partial_name(query=query)
    ...
    async def get_by_role(self, role: UserRole) -> list[UserDto]:
        users = await self.uow.repository.users.filter_by_role(role)
        return [user.dto() for user in users]

    @redis_cache(prefix="get_devs", ttl=TIME_10M)
    async def get_devs(self) -> list[UserDto]:
        devs = await self.uow.repository.users.filter_by_role(UserRole.DEV)
        return [dev.dto() for dev in devs]

    @redis_cache(prefix="get_admins", ttl=TIME_10M)
    async def get_admins(self) -> list[UserDto]:
        admins = await self.uow.repository.users.filter_by_role(UserRole.ADMIN)
    ...
    async def get_blocked_users(self) -> list[UserDto]:
        users = await self.uow.repository.users.filter_by_blocked()
        return [user.dto() for user in users]

    async def set_block(self, user: UserDto, blocked: bool) -> None:
        user.is_blocked = blocked
        await self.uow.repository.users.update(telegram_id=user.telegram_id, **user.model_state)
        await self.clear_cache(telegram_id=user.telegram_id)
